      ...
      <header className='h-[120px] flex justify-between items-center ' >
        <div className='flex justify-between items-center w-full'>
          ...
          <div className='flex items-center gap-x-5 md:gap-x-8'>
            <p className='rounded-lg px-2 py-1 text-sm text-slate-700 hover:bg-slate-100 hover:text-slate-900 capitalize hidden md:block cursor-pointer'>Signin</p>
            <CustomButton text={'Get Started Today'} />
            {
              showModal ?
                <IoCloseSharp className='cursor-pointer md:hidden h-5 w-5 z-50'
                  onClick={() => setShowModal(!showModal)} /> :
                <AiOutlineMenu className='cursor-pointer md:hidden h-5 w-5 z-50'
                  onClick={() => setShowModal(!showModal)}
                />
            }
          </div>
